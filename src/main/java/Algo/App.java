...
public class App
{
	private static int ZERO = 0, ONE = 1, TWO = 2;
	private static boolean repeat = true;
	public static void main ( String [] args)
	{
		try
		{
			int[] dd = {23, 12, 45, 6, 79, 34, 59,100,78,40,92,41,76,67,87,73,90,102,109,1000001,10001,1001,1003,112,101,159};
			Scanner rfk = new Scanner(System.in);
			for ( int a = 0; a < 13; a++){
			System.out.print("Enter Data : ");
			int datas = rfk.nextInt();
			System.out.println(trinarySeacrh(dd,datas));
			}
		}
	...
	private static <T> String trinarySeacrh (int[] data , int find)
	{
		Arrays.sort(data);
		int ofIniFirstIndex = ZERO, ofIniLastIndex = data.length-1, ofIniMiddleIndex = ( ofIniFirstIndex + ofIniLastIndex ) / TWO, ofFirstIndex = ofIniFirstIndex , ofLastIndex = ofIniLastIndex , ofMiddleIndex = ofIniMiddleIndex;
				...
				if ( find == iniMiddleData )
			{
				...
				if ( find == iniLastData )
				{
					...
					if ( find < iniMiddleData )
					{
							do {
								boolean repeat = true;
								ofFirstIndex = ofFirstIndex + ONE;
								firstData = data[ofFirstIndex];
								ofLastIndex =  ofMiddleIndex - ONE;
								lastData = data[ofLastIndex];
								ofMiddleIndex = ( ofFirstIndex + ofLastIndex ) / TWO;
